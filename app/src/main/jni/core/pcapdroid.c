...
static bool dump_payload(pcapdroid_t *pd, pd_conn_t *conn, bool is_tx, uint64_t ms, const char *to_dump, int dump_size) {
    ...
    if(pd->cb.dump_payload_chunk(pd, conn, is_tx, ms, to_dump, dump_size))
    ...
    if((pd->payload_mode != PAYLOAD_MODE_MINIMAL) || !data->has_payload[pctx->is_tx]) {
        if (pctx->plain_data) {
            ...
            for (unsigned int i = 0; i < pctx->plain_data->n_items; i++) {
                ...
                truncated |= !dump_payload(pd, pctx->data, item->is_tx, item->ms, (const char*) item->data,
                             (int) item->data_length);
            }
        } else
            truncated = !dump_payload(pd, pctx->data, pctx->is_tx, pctx->ms, pkt->l7, pkt->l7_len);
